...
TEST_PATH = ['./data/RGBD_sal/test_in_train/test_in_train']
...
conf.add_argument("--bz_size", type=int, default=4)  # 修改部分 bz_size gpu
...
def Test (test_loader, net, epoch):
    net.eval()
    global best_mae, best_epoch, best_fscore
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if best_MAE < best_mae:
                best_mae = best_MAE
                best_epoch = epoch
                torch.save(net.state_dict(), cfg.savepath + '/modal/'+'SPCNet_epoch_best.pth')
