...
class MMF(nn.Module):    #空间注意力机制，先将通道数转化为1
    def __init__(self,out_channel):
        super(MMF, self).__init__()

        self.global_avg_pool = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Conv2d(out_channel, out_channel, kernel_size=(1, 1), stride=1, bias=False),
        ...
        self.cat = BasicConv2d(7 * out_channel, out_channel, 3, padding=1)
        self.bn = nn.BatchNorm2d(out_channel)

    def forward(self, x1, x2):
        Sp0 = self.global_avg_pool(x1)
        Sp1 = self.DIL1(x1)
        Sp2 = self.DIL2(x1)
        Sp3 = self.DIL3(x1)
        Sp4 = self.DIL4(x1)
        Sp5 = self.DIL5(x1)
        Sp6 = self.DIL6(x1)
        Sp0 = F.interpolate(Sp0, size=x1.size()[2:], mode='bilinear', align_corners=True)
        x1_cat = self.cat(torch.cat((Sp0, Sp1, Sp2, Sp3, Sp4, Sp5, Sp6), 1))
        x1_refine  =  x1 + x1_cat

        return F.relu(self.bn(x1_refine * x2))


class CMAT(nn.Module):
    def __init__(self, in_channel, ratio, MF=True):
        super(CMAT, self).__init__()
        self.MF = MF
        self.CCA1 = CCA(in_channel, ratio, out_channel=256)
        self.CCA2 = CCA(in_channel, ratio, out_channel=256)
        if self.MF:
            self.MMF1 = MMF(out_channel=256)
            self.MMF2 = MMF(out_channel=256)
        else:
    ...
    def forward(self, rgb, depth, beta, gamma,gate):
        ...
        if self.MF:
            feat_1 = self.MMF1(depth, rgb)
            feat_2 = self.MMF2(rgb, depth)
        else:
            w1 = self.conv2(rgb)
            w2 = self.conv3(depth)          
...
'''
class SC(nn.Module):
    def __init__(self, in_channel, ratio = 64,norm_layer = nn.BatchNorm2d):
        super(SC, self).__init__()
        self.conv = nn.Conv2d(in_channel*2, in_channel, kernel_size=3, stride=1, padding=1)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x,y):
        F_xy = self.conv(torch.cat((x,y), dim=1))
        sc_weight = self.avg_pool(F_xy)
        sc_weight = self.block(sc_weight)
        out = x + sc_weight * y
        return out
...
class feature_fusion(nn.Module):
    def __init__(self,in_channel,ratio = 64,norm_layer = nn.BatchNorm2d):
        ...
        self.SC_DR = SC(in_channel,ratio)
        self.SC_RD = SC(in_channel,ratio)
        self.conv = nn.Conv2d(in_channel*2,in_channel,kernel_size=1,stride=1,padding=0)
        self.bn = norm_layer(256)
        
    def forward(self,rgb,depth):
        DR = self.SC_DR(rgb,depth)
        RD = self.SC_RD(depth,rgb)
        out = F.relu(self.bn(self.conv(torch.cat([DR, RD], dim=1))))
        return out
...
class Segment(nn.Module):
    ...
    def __init__(self, backbone='resnet50', norm_layer=nn.BatchNorm2d, cfg=None, aux_layers=True):
        ...
        self.sidefusion1 = feature_fusion(256,64)
        self.sidefusion2 = feature_fusion(256,64)
    ...
    def forward(self, rgb, depth):
        ...
        second3_1, second3_2   = self.cmat3(enc3_1, enc3_2,1,1, gate)
