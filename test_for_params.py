...
DATASETS = [
           './data/RGBD_sal/test/RGBD135','./data/RGBD_sal/test/NLPR','./data/RGBD_sal/test/NJUD', './data/RGBD_sal/test/SIP', './data/RGBD_sal/test/LFSD', './data/RGBD_sal/test/STEREO797', './data/RGBD_sal/test/SSD']

class Test(object):
    def __init__(self, conf, Dataset, Network):
        ## dataset
        ...
        # ## network
        self.net = Network(backbone='resnet50',  norm_layer=nn.BatchNorm2d)
    ...
    def save(self):
        with torch.no_grad():
            ...
            flops, params = thop.profile(self.net,inputs=(image, d))
            flops, params = thop.clever_format([flops, params], '%.3f')
            print('flops:', flops)
            print('params:', params)
            # out     = F.interpolate(out, size=(640,480), mode='bilinear')
            # pred     = (torch.sigmoid(out[0,0])*255).cpu().numpy()
            # head     = './CEDNetT-result/{}/'.format(self.tag) + self.cfg.datapath.split('/')[-1]
